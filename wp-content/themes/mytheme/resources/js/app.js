...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (contentElement) {
    ...
    $(document).on('click', '.prqu_minus, .prqu_plus', function(event) {
        ...
        var price_per_unit = parseFloat($(this).closest('.woocommerce-cart-form__cart-item').find('.product-price .woocommerce-Price-amount').text().replace(/[^0-9.-]+/g, ""));
        ...
        $(this).closest('.woocommerce-cart-form__cart-item').find('.product-subtotal .woocommerce-Price-amount').html(formatted_price);
    ...
    function formatPrice(price) {
        return parseFloat(price).toLocaleString('sv-SE', { style: 'currency', currency: 'SEK', minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
